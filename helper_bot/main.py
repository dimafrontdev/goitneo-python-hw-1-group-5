...
def add_contact(args, contacts):
    if len(args) != 2:
        return "\033[91mInvalid input. Please use the format 'add [name] [phone number]'\033[0m"
    name, phone = args
    contacts[name] = phone
...
def change_contact(args, contacts):
    if len(args) != 2:
        return "\033[91mInvalid input. Please use the format 'change [name] [new phone number]'\033[0m"
    name, new_phone = args
    if name in contacts:
...
def show_phone(args, contacts):
    if len(args) != 1:
        return "\033[91mInvalid input. Please use the format 'phone [name]'\033[0m"
    name = args[0]
    if name in contacts:
